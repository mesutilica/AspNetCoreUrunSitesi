...
namespace WebApi.Utils
{
    ...
    {
        public static async Task<string> FileLoader(IFormFile formFile, string filePath = "/Img/") // FileLoader metodu sunucuya resim yüklemeyi sağlar. 1. parametre formFile = yüklenecek dosya bilgisi, 2. parametre filePath= dosyanın yükleneceği varsayılan klasör(eğer filepath e bilgi gönderilmezse dosya varsayılan olarak wwwroot içindeki ımg klasörüne yüklenir).
        {
            ...
            if (formFile != null && formFile.Length > 0) // dosya gerçekten var mı ve içi dolu mu kontrolü
            {
                ...
                await formFile.CopyToAsync(stream); // parametreden gelen dosyayı ilgili klasöre kopyala
            }
